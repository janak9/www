...
class JobClass
 {
  ...
  function insertion($tbl,$data)
  {
   ...
   //print_r($data);
    ...
    if(mysqli_query($this->con,$qry) or die(mysqli_error($this->con))){
      return true;
    }else{
      return false;
    }
  ...
  function select_desc($tbl,$whr,$whid,$id){
    // it return one record of which condition is passed in the form of array.
    $qry="select * from " . $tbl . " where " . $whr . "=" . $whid." order BY ". $id ." DESC";
    $res=mysqli_query($this->con,$qry);
    return $res;
  }
  function img_upload($data,$imgpath){
    ...
    function img_upload2($data,$name,$imgpath)
    {
          $type = $data['type'];
          $tmp = substr($type,6,strlen($type));
          $ran_img=rand(100,999);
          $n=date("smy");
          $img ="cnd" . $n . $ran_img . "." . $tmp; 
          echo $path = "assets/images/" . $img;
  ...
  function abc($id) {
    echo "Php function is called via javascript";
    echo "<scrip>alert($id);</script>";
  }
}
